...
<?php $seconds2 = strtotime($month_num1 . "/1/" . $year2/* . " - 1 month"*/); /*complete next monthly date into seconds*/ ?>
                <option <?php if($seconds1==$this->uri->segment(4)) : ?>selected="selected"<?php endif ?> onclick="window.open('<?php echo base_url(); ?>admin/admin_accounting/merchant/<?php echo $seconds1; ?>/<?php echo $seconds2; ?>','_self');" value="<?php echo strtotime("January 1, " . $year1); ?>"><?php echo date($m, $seconds1); ?></option>
<?php $month=$month+1; $month_num1=$month_num1+1; ?>
<?php if($month_num1==13) { $month_num1=1; $year2=$year2+1; } ?>
<?php endfor ?>
            </select>
<?php $table = "companies"; $table1 = "deal_view"; $table2 = "companies"; $table3 = "deal_category"; $table4 = "deals"; $table5 = "deal_option"; $table10 = "deal_payment"; ?>
<?php /*sqlA variable call*/ $company_where["user_id"] = $this->session->userdata('user_id'); ?>
<?php $sqlA = $this->db->get_where($table, $company_where); ?>
<?php if($sqlA->num_rows()==0) : $where_company["company_id"] = 0; ?>
<?php else : ?> 
<?php foreach($sqlA->result() as $rowA) : $where_company["company_id"] = $rowA->company_id; endforeach ?>
<?php endif ?>
...
<?php /*sql1 variable call $company_where["deal_hash"] = "";*/ $where_start = $this->uri->segment(4); $where_end = $this->uri->segment(5); ?>
<?php $sql1 = $this->Deals_View_Model->displaySelectedDateBasedMerchant($table1, $where_company, $where_start, $where_end); ?>
        </header>          
...
<?php /*$sql4 variable vcall*/ $where["deal_hash"] = xss_cleaner($row1->deal_hash); ?>
...
<?php /*variable calls for $NPFM*/ $sp=xss_cleaner($row4->deal_discounted_price); $x=$webcall/100; $tf=(xss_cleaner($row4->deal_original_stock)-xss_cleaner($row4->deal_current_stock))+(xss_cleaner($row5->deal_original_stock)-xss_cleaner($row5->deal_current_stock)); $nr=xss_cleaner($row4->deal_returned)+xss_cleaner($row5->deal_returned); ?>
<?php $NPFM = npfm($sp, $x, $tf, $nr); ?>
                ...
                <tr id="payment_details" class="payment_details<?php echo $show_count; ?>">
                    <td></td>
                    <td title="<?php echo $row1->deal_view_title; ?>">
                        ...
                        <span class="deal deal_type"><?php echo xss_cleaner($row1->deal_view_type); ?></span>
...
<?php endif ?>
<?php if((xss_cleaner($row4->deal_original_stock)-htmlentities($row4->deal_current_stock)) == 0 && (xss_cleaner($row5->deal_original_stock)-xss_cleaner($row5->deal_current_stock)) == 0) : ?>
...
<?php endif ?>
<?php if(xss_cleaner($row4->deal_returned) == 0 && htmlentities($row5->deal_returned) == 0) : ?>
...
<?php endif ?>
<?php if(xss_cleaner($row4->deal_current_stock) == 0 && xss_cleaner($row5->deal_current_stock) == 0) : ?>
