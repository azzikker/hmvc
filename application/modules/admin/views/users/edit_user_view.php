...
<?php foreach($sql1 as $row1) : ?>
    <center style="margin-top: 100px;">
        <article class="module width_full" style="max-width: 500px;">
            <header><h3><?php echo ucfirst($row1->user_firstname); ?>'s Profile</h3></header>
            ...
            <div style="margin: 20px;">
                <div id="deal_type">
                    ...
                    <fieldset>
                        ...
                        <label>Username</label><input type="text" name="txtUN" value="<?php echo htmlentities($row1->user_name); ?>" required="required">
                    ...
                    <fieldset>
                        ...
                        <select name="txtUL" <?php if($this->session->userdata('user_level') != 3 && $this->session->userdata('user_level') != 0) : ?>disabled="disabled"<?php endif ?> required="required">
<?php foreach($sqlxb as $rowxb) :?>
                            <option value="<?php echo $rowxb->level_id; ?>" selected="selected"><?php echo $rowxb->level_name; ?></option>
<?php endforeach ?>
<?php foreach($sqlxa as $rowxa) : ?>
<?php if($rowxa->level_id==0) : ?>
<?php else : ?>
                            <option value="<?php echo $rowxa->level_id; ?>"><?php echo $rowxa->level_name; ?></option>
<?php endif ?>
<?php endforeach ?>
