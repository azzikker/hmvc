...
class Vigattin_home_model {
	...
	public function __construct()
	{
		...
		$this->CI->VIGATTIN_HOME_DB = $this->CI->load->database($VIGATTIN_HOME_DB_SETTING, TRUE);
	...
	public function add_link($name, $href, $category = 'default', $note = '')
	{
		...
		return $this->CI->VIGATTIN_HOME_DB->query($sql, array($name, $href, $category, $note));
	}
	public function remove_link($id)
	{
		$sql = "DELETE FROM `link` WHERE `id` = ? LIMIT 1";
		return $this->CI->VIGATTIN_HOME_DB->query($sql, array($id));
	}
	public function get_link()
	{
		$sql = "SELECT * FROM link ORDER BY category ASC";
		$result = $this->CI->VIGATTIN_HOME_DB->query($sql);
		$link = array();
	...
	public function encoded_link($home_link = '')
	{
		...
		if(is_array($link))
		{
			$ul = '<ul class="navigation"><li class="link home"><a target="_parent" href="'.$home_link.'">HOME</a></li>';
			foreach($link as $key => $value)
			{
				if(is_array($value))
				{
					...
					else
					{
						...
						$ul .= '<ul class="sub_navigation" style="height:0">';
						$total = count($value);
	...
	private function init_table_link()
	{
		...
		$this->CI->VIGATTIN_HOME_DB->query($query);
	}
}